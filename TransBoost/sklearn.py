...
from .training import train, train_transboost
...
class XGBClassifier(XGBModel, XGBClassifierBase):
    ...
    def evals_result(self):
        ...
        **evals_result** will contain the **eval_metrics** passed to the `fit` function.
        ...
        return evals_result
@xgboost_model_doc(
    "Implementation of the scikit-learn API for TransBoost algorithm.",
    ['model', 'objective'], extra_parameters='''
    n_estimators : int
        Number of boosting rounds.
    use_label_encoder : bool
        (Deprecated) Use the label encoder from scikit-learn to encode the labels. For new
        code, we recommend that you set this parameter to False.
    transfer_decay_ratio : float
        Balance factor for the overall weight of source and target domain. 
        The default is 1.0 (source and target domain have equal importance).
    transfer_rebalance : bool
        Adjust the weight of source domain to ensure the average weight equals 1. 
        The default is False.
    transfer_margin_estimation : string
        ['firstorder','secondorder'] Tylor's approximation to estimate the loss function.
        The default is 'secondorder'.
    transfer_min_leaf_size : float
        Minimal leaf size for believing the weight estimation of the ancillary model.
        The default is 1.0.
    transfer_prior_margin : string
        ['zero','mirror','prior','original'] method for the weight estimation when transfer_min_leaf_size is less than transfer_min_leaf_size. 
        The default is 'mirror'.
    transfer_velocity : float 
        (Deprecated) The velocity to balance the learning speed of the main and ancillary model. 
        The default is 1.0
''')
class TransBoostClassifier(XGBModel, XGBClassifierBase):
    # pylint: disable=missing-docstring,invalid-name,too-many-instance-attributes
    @_deprecate_positional_args
    def __init__(
        self,
        ...
        gpu_id=None,
        validate_parameters=None,
        transfer_decay_ratio: Optional[float] = 1.0,
        transfer_rebalance: Optional[bool] = False,
        transfer_margin_estimation: Optional[str] = 'secondorder',
        transfer_min_leaf_size: Optional[bool] = 1.0,
        transfer_prior_margin: Optional[str] = 'mirror',
        transfer_velocity: Optional[float] = 1.0,
        **kwargs
    ):
        if not SKLEARN_INSTALLED:
            raise XGBoostError(
        ...
        self.gpu_id = gpu_id
        self.validate_parameters = validate_parameters
        self.transfer_decay_ratio = transfer_decay_ratio
        self.transfer_velocity = transfer_velocity
        self.transfer_rebalance = transfer_rebalance
        self.transfer_min_leaf_size = transfer_min_leaf_size
        self.transfer_prior_margin = transfer_prior_margin
        self.transfer_margin_estimation = transfer_margin_estimation
    ...
    def fit(
        ...
        *,
        ...
        #Transboost code
        #Data initialization 
        X=pd.concat([pd.DataFrame(X_source),pd.DataFrame(X_target)])
        y=pd.concat([pd.DataFrame(y_source),pd.DataFrame(y_target)])
        sample_type=[1 for x in range(X_source.shape[0])] #0:source domain
        sample_type.extend([0 for x in range(X_target.shape[0])]) #1:target domain
        ...
        param_transboost = {
            ...
            'transfer_decay_ratio' : self.transfer_decay_ratio,
            'transfer_velocity' : self.transfer_velocity,
            'transfer_rebalance' : self.transfer_rebalance,
            'transfer_min_leaf_size' : self.transfer_min_leaf_size,
            'transfer_prior_margin' : self.transfer_prior_margin,
            'transfer_margin_estimation' : self.transfer_margin_estimation
            }
        self._Booster = train_transboost(
            params,
            train_dmatrix,
            sample_type,
            self.get_num_boosting_rounds(),
